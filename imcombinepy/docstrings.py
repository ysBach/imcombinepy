...
sigma : float-like, optional.
    The sigma-factors to be muiltiplied to the sigma values. Overridden
    by ``sigma_lower`` and/or ``sigma_upper``, if input.

sigma_lower : float or `None`, optional
    The number of standard deviations to use as the lower bound for the
    clipping limit. If `None` then the value of ``sigma`` is used. The
    default is `None`.

sigma_upper : float or `None`, optional
    The number of standard deviations to use as the upper bound for the
    clipping limit. If `None` then the value of ``sigma`` is used. The
    default is `None`.

maxiters : int, optional.
    The maximum number of iterations to do the rejection. It is silently
...
nkeep : float or int, optional.
    The minimum number of pixels that should be left after rejection. If
    ``nkeep < 1``, it is regarded as fraction of the total number of
    pixels along the axis to combine.
...
maxrej : float or int, optional.
    ...
    total number of pixels along the axis to combine.
...
irafmode : bool, optional.
        Whether to use IRAF-like pixel restoration scheme. Default is
        `True`.
...
scale_to_0th : bool, optional.
    Whether to re-scale the scales such that ``scale[0]`` is unity (in
    python, ``scale/scale[0]``). This is the behavior of IRAF, so
    ``scale_to_0th`` is `True` by default.
...
zero_kw, scale_kw : dict
    ...
    .. note::
        ...
        #. Sigma-clipping in astropy has *cumulative rejection*
           algorithm by default. This may give unwanted results
           especially when ``sigma`` in this ``zero_kw`` or
           ``scale_kw`` is small (around 1), but this is not likely
...
nkeep : float or int, optional.
    ...
    corresponds to *positive* ``nkeep`` parameter of IRAF
    `IMCOMBINE`_. If number of remaining non-nan value is fewer than
    ``nkeep``, the masks at that position will be reverted to the
...
maxrej : float or int, optional.
    ...
    rejection. If ``maxrej < 1``, it is regarded as fraction of the
    total number of pixels along the axis to combine. This
    corresponds to *negative* ``nkeep`` parameter of IRAF
    `IMCOMBINE`_. In IRAF, only one of ``nkeep`` and ``maxrej`` can
    be set. If number of rejected pixels at a position exceeds
    ``maxrej``, the masks at that position will be reverted to the
    previous iteration, and rejection code will be added by number
    8.
...
combine: str, optional.
    ...
    .. note::
        The lower median means the median which takes the lower value
        when even number of data is left. This is suggested to be robust
        against cosmic-ray hit according to IRAF `IMCOMBINE`_ manual.
...
mask_total : ndarray (dtype bool)
    ...
    by ``np.count_nonzero(mask_total, axis=0)``.
...
mask_rej, mask_thresh : ndarray(dtype bool)
    ...
    ``nrej = np.count_nonzero(mask_rej, axis=0)``. Note that
...
offsets : str or (n, m)-d array
    ...
    .. note::
        Though IRAF `IMCOMBINE`_ says it calculates offsets from the
        ...
        reproducible. Also, if we only use ``CRPIX``, the offset
        calculations are completely wrong if ``CRPIX`` is not
