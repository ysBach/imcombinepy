...
def ma_median(
    ...
    if not hasattr(a, 'mask'):
        m = median(np.ma.getdata(a, subok=True), axis=axis,
    ...
    r, k = np.lib.function_base._ureduce(
        a, func=_ma_median, axis=axis, out=out,
...
def _nanmedian_small(
        a, axis=None, out=None, overwrite_input=False, choose='mean'
):
...
def nanmedian(
    ...
    r, k = np.lib.function_base._ureduce(
        a, func=_nanmedian, axis=axis, out=out,
        overwrite_input=overwrite_input, choose=choose
    )
